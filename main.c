...
{	
	...
	key_t key = 0;
	int busy_flag = 0;
	Type tmp_line = NULL;  // we will read line as a pointer
	//инициализация файла
	char* file_name = NULL;
	...
	init_table(capacity);
	if (f != NULL) {
		// Reading file (should be a function) ...
		for(int i = 0; i < capacity; i++) {
			fread(&key, sizeof(int), 1, f);
			fread(&busy_flag, sizeof(int), 1, f);
			tmp_line = get_line(f);
			availability = insert(key, capacity, tmp_line);
		}
		// I am ignoring size and busy
		/*		if (availability == -1)
					duplicated_key();
				if (availability == -2)
					full_table(); */
